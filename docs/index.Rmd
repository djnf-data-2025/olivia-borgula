...
# Plot the line chart
ggplot(total_long, aes(x = year, y = quantity, color = country)) +
  geom_line(size = 1) +
  geom_point() +
  labs(
    title = "Total Quantity Imported by Year and Country",
    x = "Year",
    y = "Total Quantity"
  ) +
  theme_minimal()
...
monthly_2025 <- x2020_x2025_imports %>%
  filter(year == 2025) %>%
  group_by(country, month) %>%
  summarize(total_quantity = sum(quantity, na.rm = TRUE), .groups = "drop")

ggplot(monthly_2025, aes(x = month, y = total_quantity, color = country)) +
  geom_line(size = 1) +
  geom_point() +
  scale_x_continuous(breaks = 1:12, labels = month.abb) +
  labs(
    title = "Monthly Import Trend in 2025 by Country",
    x = "Month",
    y = "Total Quantity"
  ) +
  theme_minimal()
...
Some findings: 
- Cotton T-shirt imports from China increased between 2023 and 2024, but have decreased every month of 2025
- Cotton T-shirt imports from Honduras have increased in 2025 
